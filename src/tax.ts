...
export class Tax {
    ...
    private async makeApiCall(url: string): Promise<any> {
        ...
        try {
            ...
            throw new TaxNetworkError(`Failed to fetch tax data: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        ...
        try {
            ...
            throw new TaxApiError(`Invalid JSON response: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
