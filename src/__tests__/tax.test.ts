...
describe('Tax', () => {
  ...
  describe('API error handling', () => {
    ...
    it('should handle malformed JSON response', async () => {
      ...
      const tax = new Tax(25000);
      await expect(tax.getDeduction()).rejects.toThrow('Invalid JSON response'); // Changed to match new error message
    });
    ...
    it('should handle malformed JSON response with specific error', async () => {
      ...
      await expect(tax.getDeduction()).rejects.toThrow(
    ...
    it('should handle non-Error objects in catch block', async () => {
      ...
      const tax = new Tax(25000);
      await expect(tax.getDeduction()).rejects.toThrow('Failed to fetch tax data: Unknown error');
    });

    it('should handle Error objects in JSON parsing', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
      ...
      const tax = new Tax(25000);
      await expect(tax.getDeduction()).rejects.toThrow('Invalid JSON response: Parse failed');
    });
  ...
  describe('period handling', () => {
    it.each([
      ...
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining(expectedValue));
    });
  });
  ...
  describe('tax table validation', () => {
    it('should throw error for invalid table/year combination (8150 in 2024)', () => {
      expect(() => new Tax(50000, '8150', 'Wage', 'Monthly', 2024)).toThrow(
        'Invalid tax table "8150" for year 2024'
      );
    });

    it('should throw error for invalid table/year combination (7100 in 2025)', () => {
      expect(() => new Tax(50000, '7100', 'Wage', 'Monthly', 2025)).toThrow(
        'Invalid tax table "7100" for year 2025'
      );
    });

    it('should accept valid table for 2024', () => {
      expect(() => new Tax(50000, '7100', 'Wage', 'Monthly', 2024)).not.toThrow();
    });

    it('should accept valid table for 2025', () => {
      expect(() => new Tax(50000, '8150', 'Wage', 'Monthly', 2025)).not.toThrow();
    });
  });
  ...
  describe('toString', () => {
    ...
    it('should handle 2025 tax table', () => {
      const tax = new Tax(50000, '8150', 'Wage', 'Monthly', 2025);
  ...
  describe('Symbol.toStringTag', () => {
    ...
    it('should work with default values', () => {
      ...
      const result = tax[Symbol.toStringTag]();
      expect(result).toBe(
        'Tax(gross_income=0, tax_table="7100", income_type="Wage", period="Monthly", year=2024)'
      );
    });
    ...
    it('should handle 2025 tax table', () => {
      ...
      const result = tax[Symbol.toStringTag]();
      expect(result).toBe(
        'Tax(gross_income=50000, tax_table="8150", income_type="Wage", period="Monthly", year=2025)'
      );
    });
  });
});
